...
bool SSIDDatabase::autoDiscover() {
    if (LittleFS.begin()) {
        if (LittleFS.exists("/ssid_list.txt")) {
            currentFilename = "/ssid_list.txt";
            useLittleFS = true;
            LittleFS.end();
            return loadFromFile();
        }
    ...
    if (SD.begin()) {
        ...
        if (SD.exists("/ProbeData/ssid_list.txt")) {
            currentFilename = "/ProbeData/ssid_list.txt";
            useLittleFS = false;
            SD.end();
            return loadFromFile();
        }
...
void ActiveBroadcastAttack::broadcastSSID(const String &ssid) {
    sendBeaconFrameDirect(ssid, currentChannel);
}
...
void updateChannelDisplay() {
    ...
    tft.fillRect(tftWidth - 80, tftHeight - 18, 70, 12, bruceConfig.bgColor);
    tft.setCursor(tftWidth - 80, tftHeight - 18);
    tft.print("Ch." + 
        String(karma_channels[channl % 14] < 10 ? "  " : 
    ...
    std::vector<Option> loadOptions = {
        {"From SD Card", [=]() {
            ...
            if (setupSdCard()) {
                ...
                if (templateFile.length() > 0) {
                    ...
                    customTmpl.name = "[SD] " + getFileNameFromPath(templateFile);
                    ...
                    displayTextLine("Loaded: " + getFileNameFromPath(templateFile));
                    delay(1500);
                    if (isInitialSetup) {
                        drawMainBorderWithTitle("KARMA SETUP");
                        displayTextLine("Template loaded from SD");
                        delay(1000);
                    }
                } else {
                    display_clear();
                    drawMainBorderWithTitle("CANCELLED");
        ...
        {"From LittleFS", [=]() {
            ...
            if (LittleFS.begin()) {
                ...
                if (templateFile.length() > 0) {
                    PortalTemplate customTmpl;
                    customTmpl.name = "[FS] " + getFileNameFromPath(templateFile);
                    customTmpl.filename = templateFile;
                    customTmpl.isDefault = false;
                    customTmpl.verifyPassword = false;
                    ...
                    if (isInitialSetup) {
                        drawMainBorderWithTitle("KARMA SETUP");
                        displayTextLine("Template loaded from FS");
                        delay(1000);
                    }
                } else {
                    display_clear();
...
bool selectPortalTemplate(bool isInitialSetup) {
    ...
    templateOptions.push_back({"Load Custom File", [=]() {
        if (isInitialSetup) {
            handleCustomFileLoad(true);
        } else {
            handleCustomFileLoad(false);
        }
    }});
...
void updateKarmaDisplay() {
    ...
    if (currentTime - last_time > 1000) {
        ...
        if (broadcastAttack.isActive()) {
            tft.fillRect(tftWidth - 150, tftHeight - 100, 140, 10, bruceConfig.bgColor);
...
void karma_setup() {
    ...
    if (setupSdCard()) {
        ...
        if (!SSIDDatabase::autoDiscover()) {
            displayTextLine("No SSID database");
        } else {
            displayTextLine(String(SSIDDatabase::getCount()) + " SSIDs");
        }
    } else {
        Fs = &LittleFS;
        FileSys = "LittleFS";
            ...
            if (!redraw && !redrawNeeded) {
                std::vector<Option> options = {
                    ...
                    {"Karma Attack", [=]() {
                         ...
                         karmaOptions.push_back({"Back to Menu", [=]() {
                             returnToMenu = false;
                             redrawNeeded = true;
                         }});
                    ...
                    {"Active Broadcast Attack", [=]() {
                        ...
                        broadcastOptions.push_back({"Database Info", [=]() {
                            ...
                            tft.setCursor(10, y); y += 15;
                            tft.print("Source: " + String(SSIDDatabase::useLittleFS ? "LittleFS" : "SD"));
                            tft.setCursor(10, y); y += 15;
                            tft.print("File: " + SSIDDatabase::getSourceFile());
                            if (total > 0) {
                                tft.setCursor(10, y); y += 15;
                        ...
                        broadcastOptions.push_back({"Manage Database", [=]() {
                            std::vector<Option> dbOptions = {
                                {"Load from SD", [=]() {
                                    if (setupSdCard()) {
                                        String ssidFile = loopSD(SD, true, "TXT", "/");
                                        if (ssidFile.length() > 0) {
                                            if (SSIDDatabase::setSourceFile(ssidFile, false)) {
                                                displayTextLine("Loaded: " + String(SSIDDatabase::getCount()) + " SSIDs");
                                            } else {
                                                displayTextLine("Failed to load");
                                            }
                                        } else {
                                            displayTextLine("No file selected");
                                        }
                                ...
                                {"Load from LittleFS", [=]() {
                                    if (LittleFS.begin()) {
                                        String ssidFile = loopSD(LittleFS, true, "TXT", "/");
                                        if (ssidFile.length() > 0) {
                                            if (SSIDDatabase::setSourceFile(ssidFile, true)) {
                                                displayTextLine("Loaded: " + String(SSIDDatabase::getCount()) + " SSIDs");
                                            } else {
                                                displayTextLine("Failed to load");
                                            }
                                ...
                                {"Auto Discover", [=]() {
                                    if (SSIDDatabase::autoDiscover()) {
                                        displayTextLine("Found: " + String(SSIDDatabase::getCount()) + " SSIDs");
                                    } else {
                                        displayTextLine("No database found");
                                    }
                                    delay(1000);
                                }},
                                {"Current Info", [=]() {
                                    String source = SSIDDatabase::useLittleFS ? "LittleFS" : "SD Card";
                                    String file = SSIDDatabase::getSourceFile();
                                    size_t count = SSIDDatabase::getCount();
                                    display_clear();
                                    drawMainBorderWithTitle("DATABASE INFO");
                                    displayTextLine("Source: " + source);
                                    displayTextLine("File: " + file);
                                    displayTextLine("SSIDs: " + String(count));
                                    delay(2000);
                                }},
                                {"Add Current Probes", [=]() {
